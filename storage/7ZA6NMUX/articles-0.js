...
function initDownloadTo () {
	jQuery('#download-to-links .iconlist').attr("id", "downloadto").hide();//add the id to apply the css style attached and hides the box
	//jQuery('.iconlist li:even').addClass('left');
	//jQuery('.postto_title').replaceWith('<a href="#" class="downloadto_title">Share this article</a>');
	jQuery('a.downloadto_title')
		.hover(function() {$('#downloadto').stop(true, true).show();},function () {$('#downloadto').fadeOut();})
		.click(function(){return false;})//disable the link
	jQuery('#downloadto').hover(function() {$(this).stop(true, true).show();  $("a.downloadto_title").addClass("btn-on");},function() {$('#downloadto').stop(true, true).hide(); $("a.downloadto_title").removeClass("btn-on")});//fades out after 1s out of the box
	//push down the bottom ad when the central bloc (content) is too short
	contentOffset = jQuery('.article').height() - (jQuery('#article-navigation-bar').height() + jQuery('#downloadto').height());
	if(contentOffset < 0) {
		jQuery('#content').css({'padding-bottom':jQuery('#downloadto').height() + 'px'});
	}
}

function initShareThis () {
	jQuery('#social-networking-links .iconlist').attr("id", "sharethis").hide();//add the id to apply the css style attached and hides the box
	//jQuery('.iconlist li:even').addClass('left');
...
function outlineAbbreviations() {
	window.onscroll = document.documentElement.onscroll = function() { setMenuOffset();  advertOffset();  };
...
function advertOffset() {
	var scroll = (window.pageYOffset || document.body.scrollTop || document.documentElement.scrollTop);//scroll position (cross-browser)
	var adBox = jQuery("#advert-offset");//box to move
	var adBoxTop = scroll + 20;//20 = leftnav's css "top" property
	var adBoxBottom = adBoxTop + adBox.height();
	var adcontentTop = jQuery("#signup-to-etoc").offset().top + jQuery("#signup-to-etoc").height() + 21;//vertical position FROM which the position will be fixed
	var adcontentBottom = adcontentTop + jQuery(".full-text").height();//vertical position TO which the position will be fixed
	
	if (adBoxTop > adcontentTop)
		if (adBoxBottom > adcontentBottom) {
			var hackTop = adcontentBottom - adBox.height();
			adBox.css({'position':'absolute','top':hackTop + 'px'});
		}
		else if((navigator.userAgent.match(/iPhone/i)) || (navigator.userAgent.match(/iPad/i))){
			adBox.css({'position':'relative','top':'20px'});
		}
		else
			adBox.css({'position':'fixed','top':'20px'});		
	else
		adBox.css({'position':'absolute','top':''});
...
jQuery(document).ready(function() {
	...
	if(jQuery('.article').offset() && (document.cookie.match("mobile") == null)) { initAffiliations(); }
	

	if (jQuery('#sharethis').offset() && $("body").hasClass("mobile") != true) { initShareThis(); }
	if (jQuery('#downloadto').offset() && $("body").hasClass("mobile") != true) { initDownloadTo(); }
 
	if (!($.browser.webkit) && !!window.location.search.match("fmt_math=no") == false){
		if (jQuery('.mathimg').offset()) { 
		if ((site.page == "abstract" && site.portalId != '9014') || site.page == "fulltext"){
		mathmlLightBox();
		}
		}
	}
...
function popInJmol(id, scr)
{
    ...
    jmolInitialize("/includes/jmol")
    jmolSetDocument(false)
    document.getElementById(id).innerHTML = jmolApplet(500, 'load' + scr)
}


// temporary for testing embedded mini website
$(document).ready(function() {
	if($(".embedded-mini-website").length > 0 && document.cookie.match("mobile") == null) {
		$("div#content").width("1300px")
		$("div.article.full-text").width(50 + $(".embedded-mini-website").width() + "px")

		// resizing iframe containers
		var emb = {
			"/content/supplementary/1758-2946-5-6-s1/": [739, 894],
			"/content/supplementary/1758-2946-5-6-s2/": [553, 570],
			"/content/supplementary/1758-2946-5-6-s3/": [283, 558],
			"/content/supplementary/1758-2946-5-6-s4/": [600, 580]
		}

		$(".embedded-mini-website").each(function(e) {

			var p = $(this).parents("p");
			$(this).detach().prependTo(p);

		    $(this).attr("width", emb[$(this).attr("src")][1] + 20);
		    $(this).attr("height", emb[$(this).attr("src")][0] + 35);
		    $(this).attr("style", "background-color: #f7f7f7; display: block;");

		});			
	}	
});

// rezize article to fit embedded silverlight
$(document).ready(function() {
	if($('object a').attr('class') == 'silverlight'){
		$("div.article.full-text").css("width", "900");
		$("#content").css("width", "1260px");
	}
});

// Facebook recommend buttton
(function(d, s, id) {
