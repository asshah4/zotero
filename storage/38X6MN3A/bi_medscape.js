var s_ver="medscape|20100111",s_live=false,_ud="undefined";
try { if (s_account == "webmddev") { s_account="webmdp1medscapevistadev";s_live=false;} 
	else {s_account="webmdp1global"; s_live=true;}
} catch(e) {var s_account="webmdp1global"; s_live=true; }
var _src=_readQ("src");
if(_src!=""){_src=_src.toLowerCase();}
var s_md=s_gi(s_account)
_dom = document.domain.split('.');
var s_domain=_dom[_dom.length-2] + '.' + _dom[_dom.length-1];
s_md.cookieDomainPeriods="3"
s_md.jsCookieDomainPeriods="2"
s_md.charSet="ISO-8859-1"
s_md.currencyCode="USD"
s_md.trackDownloadLinks=true
s_md.trackExternalLinks=true
s_md.trackInlineStats=true
s_md.linkDownloadFileTypes="exe,zip,wav,mp3,mov,mpg,avi,wmv,doc,pdf,xls"
s_md.linkInternalFilters="javascript:,.medscape.com"
s_md.linkLeaveQueryString=false
s_md.linkTrackVars="prop50"
s_md.linkTrackEvents="None"
s_md.visitorNamespace="webmd"
s_md.usePlugins=true;
function s_md_doPlugins(s) {
	if (!s_md.pageName) {
		s_md.pageName=location.protocol + "//" + location.host + location.pathname;
	}
}
s_md.doPlugins=s_md_doPlugins
s_md.c_gd=new Function(""
+"var s=this,d=s.wd.location.hostname,n=s.jsCookieDomainPeriods,n2=s."
+"cookieDomainPeriods,p;if(d&&!s.c_d){n=n?n:n2;n=n?parseInt(n):2;n=n>"
+"2?n:2;p=d.lastIndexOf('.');while(p>=0&&n>1){p=d.lastIndexOf('.',p-1"
+");n--}s.c_d=p>0&&s.pt(d,'.','c_gdf',0)?d.substring(p):'';}return s."
+"c_d");
s_md.trackGAS=new Function(""
+"var s_md=this,gifa=document.getElementsByTagName('iframe'),gif;for(v"
+"ar i=0;i<gifa.length;i++){if(gifa[i].src.indexOf('googlesyndication."
+"com')>-1){gif=gifa[i];if(gif.attachEvent){gif.attachEvent('onfocus',"
+"s_md.gasf);}}}");
s_md.gasf=new Function(""
+"var s_md=s_gi(s_account);s_md.prop36=s_md.pageName;s.tl(true,'o','Go"
+"ogle Exit Link');");
/* Google AdSense Tracking */
s_md.trackGAS();
/* WEBMD code for Medscape start here */
var s_site="medscape", s_subdomain="";
if(typeof _dom!=_ud && _dom.length>=2)
{
	if(_dom[1]=="imedicine" || _dom[0]=="imedicine"){s_site="imedicine";s_subdomain="imedicine";}
	else{
		if(_dom[0]=="cme"){s_site="cme";}
		else if(_dom[0]=="emedicine"){s_site="emedicine";}
		s_subdomain=_dom[0];
	}
}

function _clean(a) {
	//a=a.split("/").join("-");a=a.split("&").join("-");a=a.split("?").join("-");
	a=a.toLowerCase();
	return a;
}
function _urlClean(a) {
	a=a.toLowerCase();a=a.split("https://www.").join("");a=a.split("http://www.").join("");
	a=a.split("https://").join("");a=a.split("http://").join("");a=a.split("www.").join("");
	a=a.split(".html").join("");a=a.split(".htm").join("");a=a.split(".aspx").join("");a=a.split(".asp").join("");
	a=a.split("default").join("");
	a=a.split(" ").join("-");
	return a;
}
function wmdTrack(sSource){ctrs(sSource);}
function ctr(sSource) {
	var _dt=new Date()
	var dExpiry=new Date(_dt.getFullYear(), _dt.getMonth(), _dt.getDate(), _dt.getHours(), _dt.getMinutes(), _dt.getSeconds()+20);
	var sCtr=sSource;
	try { sCtr = s_md.pageName.split("_").join("-")+"_" + sSource + "|" + s_query;} catch (e) {}
	sCtr=sCtr.toLowerCase();
	document.cookie = "ctr=" + sCtr  + "; path=/; domain=" + s_domain + ";expires=" + dExpiry.toGMTString();
}
function ctrs(sSource){
	var _dt=new Date()
	var dExpiry=new Date(_dt.getFullYear(), _dt.getMonth(), _dt.getDate(), _dt.getHours(), _dt.getMinutes(), _dt.getSeconds()+20);
	var sCtr=sSource;
	try { sCtr=s_md.pageName.split("_").join("-")+"_" + sSource;}catch (e){}
	sCtr=sCtr.toLowerCase();
	document.cookie = "ctr="+sCtr+"; path=/; domain="+s_domain+";expires="+dExpiry.toGMTString();
}
function wmdSearchTrack(sSource) {ctr(sSource);}
function wmdPageLink(link)
{
	var s_md;
	s_md=s_gi(s_account);
	s_md.linkTrackVars="prop15,eVar17,prop17,prop18,eVar20,prop20,prop21,prop34,prop50";	
	...
	try{s_md.eVar17=s_md.prop17;}catch(e){}
	link=s_md.pageName.split("_").join("-") + "_"+link;
	try{s_md.prop20=trun20(link);}catch(e){}
	try{s_md.eVar20=s_md.prop20;}catch(e){}
	try{s_md.prop34=s_md.pageName.split("_").join("-") +"_"+s_md.prop17;s_md.prop34=trun34(s_md.prop34);}catch(e){}
	void(s_md.tl(true, 'o', link.toLowerCase()));
	s_md.linkTrackVars="";
	s_md.prop15="";
	s_md.linkTrackVars="prop50";
}
function wmdPageview(sPageName,iCount)
{
	var s_md;
	s_md=s_gi(s_account);
	s_md.eVar17="";s_md.eVar20="";s_md.prop20="";s_md.prop18="";s_md.prop17="";s_md.prop34="";
	s_md.prop9=iCount;
	try{sPageName=_urlClean(sPageName);}catch(e){};
	if(sPageName!=""){
		s_md.pageName=sPageName;s_md.eVar1=sPageName;}

	var sCtrRead = "";
	try{
		sCtrRead = String(_readC("ctr"));
		prop29LastPg = String(_readC("prop29")).toLowerCase();
		if (sCtrRead && sCtrRead != _ud){
			var _dtDel=new Date();
			document.cookie="ctr=undefined; path=/; domain=" + s_domain + ";expires=" + _dtDel.toGMTString();
			document.cookie="prop29=undefined; path=/; domain=" + s_domain + ";expires=" + _dtDel.toGMTString();
			var _CtrRead=sCtrRead.split("|");
			var s_ctr_loc=_CtrRead[0];
			var s_ctr_query=_CtrRead[1];
			var _pos=s_ctr_loc.indexOf("_")
			if (_pos>0 ){var s_ml = s_ctr_loc.substr(_pos+1);
				_pos=s_ml.indexOf("_");
				var s_mId=s_ml;
				if(_pos>0){s_mId=s_ml.substr(0,s_ml.indexOf("_"));}}
			_ctr = s_ctr_loc.toLowerCase().split('_');
			try{s_md.prop17=s_mId.toLowerCase();s_md.eVar17=s_md.prop17;}catch(e){}
			try{s_md.prop18=s_ml.toLowerCase();}catch(e){}
			if(typeof s_md.prop15==_ud|| s_md.prop15==""){
			s_md.prop15=((typeof prop29LastPg!=_ud&& prop29LastPg!="") && (typeof s_md.prop18!=_ud&& s_md.prop18!=""))? prop29LastPg+"_"+s_md.prop18:"";}
			try{s_md.prop20=trun20(s_ctr_loc).toLowerCase();s_md.eVar20=s_md.prop20;}catch(e){}
			try{s_md.prop34=_ctr[0]+'_';s_md.prop34=s_md.prop34+_ctr[1];s_md.prop34=trun34(s_md.prop34);}catch(e){}
			}}catch(e){}

	void(s_md.t());
}
function wmdPageLinkIframe(link)
{
	var s_md;
	s_md=window.parent.s_gi(window.parent.s_account);
	s_md.linkTrackVars="prop17,prop18,prop20,prop34,prop50";
	try{s_md.prop18=link;}catch(e){}
	try{s_md.prop17=link.split('_')[0];}catch(e){}
	link=window.parent.s_md.pageName.split("_").join("-")+"_"+link;
	try{s_md.prop20=trun20(link);}catch(e){}
	try{s_md.prop34=window.parent.s_md.pageName.split("_").join("-") +"_"+s_md.prop17;s_md.prop34=trun34(s_md.prop34);}catch(e){}
	void(window.parent.s_md.tl(true, 'o', link.toLowerCase()));
}
function wmdPageviewIframe(sPageName,iCount)
{
	var s_md;
	s_md=window.parent.s_gi(window.parent.s_account);
	s_md.prop20="";s_md.prop18="";s_md.prop17="";s_md.prop34="";
	s_md.prop9=iCount;
	try{sPageName=_urlClean(sPageName);}catch(e){};
	if(sPageName!=""){
		s_md.pageName=sPageName;}
	void(window.parent.s_md.t());
}
function wmdTrackLink(sSource){trun20(ctrs(s_md.pageName+"_"+sSource));}

function trun20(link){try{
	if(typeof link==_ud||link=="")return;
	if(link.length>100){
	var seg=link.split("_");
	var pName=(seg[0].length>73)?seg[0].substring(0, 72):seg[0];
	var mID=(seg[1].length>20)?seg[1].substring(0, 19):seg[1];
	if(seg.length==3){return (pName+"_"+mID+"_"+seg[2]);}
	if(seg.length==4){return (pName+"_"+mID+"_"+seg[2]+"_"+seg[3]);}}
	else{return link;}}
	catch(e){return link;}
}
function trun34(link){try{
	if(typeof link==_ud||link=="")return;
	if(link.length>100){
	var seg=link.split("_");
	var pName=(seg[0].length>79)?seg[0].substring(0, 78):seg[0];
	var mID=(seg[1].length>20)?seg[1].substring(0, 19):seg[1];
	return (pName+"_"+mID);}
	else{return link;}}
	catch(e){return link;}
}
function _readC(name)
{
	var qName = name + "=";
	...
	for(var i=0;i < ca.length;i++)
	{
		...
		if (c.indexOf(qName) == 0) return c.substring(qName.length,c.length);
	}
	return "";
}
function _writeC(sName, sValue)
{
   date = new Date(3000, 1, 1, 0, 0, 0);
...
{
	var qName = name.toLowerCase() + "=";
	if (document.URL.indexOf("?")==-1)
	{
		return "";
	}
	var _url = document.URL.split('?');
	var ca = String(_url[1]).toLowerCase().split('&');
	for(var i=0;i < ca.length;i++)
	{
		var c = ca[i];
		while (c.charAt(0)==' ') c = c.substring(1,c.length);
...
function _readRefQ(name)
{
	...
	var _url = document.referrer.split('?');
	var ca = String(_url[1]).toLowerCase().split('&');
	for(var i=0;i < ca.length;i++)
	{
		var c = ca[i];
...
var _URL = "";
try{ _URL=s_pagename; } catch (e){}
try{ _URL=s_pageName; } catch (e){}
if (_URL=="") {
  _URL1=String(document.location).split("?"); 
  _URL2=_URL1[0].split(";");
  _URL=_URL2[0]; 
}
_URL=_urlClean(_URL);
...
_asset = String(_clean(_asset));

if (typeof s_query==_ud)
{
	var s_query="";
	var s_filter="";

	if (!s_query){ 
		s_query = _readQ("querytext");
		if (s_query != "") {
			s_filter=s_query;
		}
		if(typeof s_pagenum == _ud){
			var s_pagenum="1";}
	}
}

try{if(_readQ("pagenumber")>1){s_pagenum=_readQ("pagenumber");}}catch(e){}

var _prop47fromPage="";

if(!s_user_origin){var s_user_origin="";}
if(_readC("medauth")!=""){
	//logged in user
	if(s_user_origin.toLowerCase()=="us"){
    try{ if(s_user_group.toLowerCase()=="physician") {  _prop47fromPage="us-physician"; } else{_prop47fromPage="us-other";}}catch(e){}
	}else{
		 try{ if(s_user_group.toLowerCase()=="physician") {  _prop47fromPage="non-us-physician"; } else{_prop47fromPage="non-us-other";}}catch(e){}
	}
	s_md.eVar4=_prop47fromPage;
}else if(_readC("medpermauth")!=""){
	//not logged in user
	s_user_group="undefined";
	s_user_origin="undefined";
}
else{
  //cookies are not accepted user
  s_user_group="undefined";
  s_user_origin="undefined";
 }

s_md.hier1="webmd,pro,oop,"+s_site+",-";
s_md.hier2="pro,"+s_subdomain+",medscape,-";

var s_suit="webmdp1medscapevista";
if(s_subdomain=="www" && s_site=="medscape"){s_suit="webmdp1medscapevista,webmdmedwwwvista";}
else if(s_site=="cme"){s_suit="webmdp1medscapevista,webmdmedcmevista";}
else if(s_site=="emedicine"){s_suit="webmdp1medscapevista,webmdp1emedicinevista";}
else if(s_site=="imedicine"){s_suit="webmdp1imedicinevista";}
s_md.prop50=s_suit;

try{if (s_not_pageview) {if (s_not_pageview!="n") {s_not_pageview="y";}} else {var s_not_pageview="n";}} 
catch (e){var s_not_pageview="n";}

if(!s_print){
	if(_readQ("prt")=='true'){var s_print=1;}
	if(_URL.indexOf('_print')>0){var s_print=1;}
}

var s_errAs="",s_eUrl="",s_eRef="";
try{if(!s_error){s_error="";}}catch(e){s_error="";}
try{if(s_pageType) {s_error=s_pageType;}}catch(e){s_pageType="";}
try{if(s_error!=""){_rf=document.location.href;var _qrypos=_rf.indexOf("?");}
	if (s_error==""){_rf1=document.location.href;var _qrypos1=_rf1.indexOf("ReqURL=");
	if(_qrypos1 >= 0){if (_qrypos >= 0){_qry=_rf.substring(_qrypos+1);_rf=_qry.split("ourl=").join("");_er=_rf.split("&referer=");s_eUrl=_er[0];s_eRef=_er[1];}
	_qry1 = _rf1.substring(_qrypos1+7);
	_er1 = _qry1.split("&referer=");
	s_error="errorPage";
	s_eUrl="404;http://"+document.domain+_er1[0];}}
	if(s_eUrl){s_errAs=s_eUrl.split("://").join("");
	var _len=s_errAs.indexOf("/");
	
	s_errAs=s_errAs.substring(_len,s_errAs.length);
	s_errAs=_urlClean(s_errAs).substr(1);}}catch(e){s_error=""}
	...
	try{
		...
		if (sCtrRead && sCtrRead != _ud){
			...
			if (_pos>0 ){var s_ml = s_ctr_loc.substr(_pos+1);
				...
				if(_pos>0){s_mId=s_ml.substr(0,s_ml.indexOf("_"));}}
			_ctr = s_ctr_loc.toLowerCase().split('_');
			s_md.prop34=_ctr[0]+'_';
			s_md.prop34=s_md.prop34+_ctr[1];
			s_md.prop34=trun34(s_md.prop34);}}catch(e){}}

s_md.events="event5";
s_md.pageName=_URL;
try{s_md.prop1="pro";}catch(e){}
try{s_md.prop2="oop";}catch(e){}
try{s_md.prop3=s_site.toLowerCase();}catch(e){}
try{s_md.prop4=s_channel;s_md.channel=s_channel;}catch(e){}
try{s_md.prop5=s_asset_type.toLowerCase();}catch(e){}
try{s_md.prop6=(typeof s_asset_class != _ud && s_asset_class != "")? s_asset_class.toLowerCase(): s_business_reference.toLowerCase();}catch(e){}
try{s_md.prop7=_asset.toLowerCase();}catch(e){}
try{s_md.prop8=s_pub.toLowerCase();}catch(e){s_md.prop8="-";}
try{s_md.prop9=s_pagenum;}catch(e){}
try{s_md.eVar1=s_md.pageName;}catch(e){}
try{s_md.prop10=s_query.toLowerCase();s_md.eVar10=s_md.prop10;}catch(e){}
try{s_md.prop11=s_filter;}catch (e) {}
try{s_md.prop12=s_results.toLowerCase();}catch(e){}
try{s_md.prop15=(_src.indexOf("rss")>=0)?"rss":""}catch(e){}
try{s_md.prop17=s_mId.toLowerCase();s_md.eVar17=s_md.prop17;}catch(e){}
try{s_md.prop18=s_ml.toLowerCase();}catch(e){}
try{s_md.prop19=s_user_group.toLowerCase();}catch(e){s_md.prop19="ntc";}
try{s_md.prop20=trun20(s_ctr_loc).toLowerCase();s_md.eVar20=s_md.prop20;}catch(e){}
try{s_md.prop21=_readRefQ("queryText");}catch(e){}
try{s_md.prop23=s_board.toLowerCase();}catch(e){}
try
{
	if (s_error!="")
	{
		s_md.pageType="errorPage";
			...
			if(s_eUrl)
			{
				s_md.pageName=s_eUrl;s_md.prop7=s_errAs;s_md.pageURL=s_eUrl;
			}
	}
}catch(e){}
try{s_md.prop28=s_sponsor_brand.toLowerCase();}catch(e){s_md.prop28="ntc";}
try{s_md.prop29=s_sponsor_program.toLowerCase();}catch(e){s_md.prop29="ntc";}
try{s_md.prop30=s_package_type.toLowerCase();}catch(e){s_md.prop30="ntc";}
try{s_md.prop31=s_package.toLowerCase();}catch(e){s_md.prop31="ntc";}
try{s_md.prop35=s_board_group.toLowerCase();}catch(e){}
try{s_md.prop36=s_board_topic.toLowerCase();}catch(e){}
try{s_md.prop37=s_user_origin.toLowerCase();}catch(e){}
try{s_md.prop38=s_user_specialty.toLowerCase();}catch(e){}
try{s_md.prop39=s_board_clinical.toLowerCase();}catch(e){}
if(_src!=""){sCookie("src",_src);s_md.prop43=_src; s_md.eVar43=_src;s_md.campaign=_src;}
else{if(_readC("src")!="")s_md.prop43=_readC("src");}
try{s_md.eVar0=s_md.prop43;}catch(e){}
try{s_md.prop47=s_registered_user_id.toLowerCase();}catch(e){s_md.prop47="000000000";}
try{s_md.server=s_ver.toLowerCase();}catch(e){}
/* End of WebMD Code */
...
if (s_not_pageview=="n") {var s_code=s_md.t();if(s_code)document.write(s_code);}