  ...
  buildAccessLinks: function(scope) {
    ...
    if(!SDM.entitled || (DynamicArtContent.dynamicComponents.CorpReqAvail != undefined  && !SDM.ignoreForceAbst)){
        ...
        if ( DynamicArtContent.dynamicComponents.AnonLinkAvail != undefined ) {
            ...
            accessHTML += '<a title="Click to see if you have full text access" class="corpReq cLink blueButton" queryStr="?_fmt=full" href="'
        ...
        if ( DynamicArtContent.dynamicComponents.CorpReqAvail != undefined ) {
            accessHTML += '<div class="acol1">';  
            accessHTML += '<a title="Click to see if you have full text access" class="corpReq cLink lightGreenBtn" queryStr="?_fmt=full" href="'
            accessHTML += SDM.urlPrefix + '/article/pii/' + SDM.pm.pii;
            accessHTML += '">View full text</a>';
            accessHTML += '</div>';
            col1=true;
        }
...
var Fragment = {
  ...
  init: function() {
    ...
    if (typeof SDM.pm.frag.mapsStr == 'undefined') {
      ...
      var mapIntrvl = setInterval(mapCheck, 0);
    } else {
  ...
  processScroll: function (visTop, visBot, isDown) {
    ...
    if(isDown) {
      if($('#frag_' + fList[fList.length-1]).attr('data-last')) {}
        ...
        DBG.out(1, 'Going down... looking for ' + nfs);
        if(!$(nfs).length) {
...
//REBASE        $(".btnHolder").bind('click',function(e){page.toggleOptions(e,this);} );
        ...
        $('#leftPane, #leftPaneInner, #rightPane, #leftCloseBar, #centerPane, #centerContent, #rightInner').addClass('js');
        $('#outline, #olGraphCbBox').addClass('js');
      ...
      startOutline: function() {
        ...
        if (SDM.tocCacheAvail) {
          if(typeof SDM.tocJson != 'undefined') {
            ...
            ///////////
            var tocIntrvl = setInterval(tocCheck, 0);
          }
        }
        else {
    ...
    if($.browser.msie && $.browser.version <= 6) {return page;}
    ...
    if($('#frag_1').length || !Fragment.isAvail()) {
      ...
      Fragment.get('maps');
      Fragment.get('frag_1', null, {successCb: page.addJS});
    }
    return page;
  }();
...
var GadgetUtil = {
  ...
  loadGadgets:function() {
    ...
    gsURL = SDM.urlPrefix +'/gs/'+SDM.pm.pii+'/'+e;
    $.get(gsURL, function(data){
      if(data) {
        try {
      ...
      if(SDM.sciverseDecoded==true && sciverse.length > 0 && sciverse.indexOf("gadgetId") != -1) {
  ...
  loadGadgetsParallel:function() {
    ...
    $.get(gadgetURL, function(data) {
      ...
      if(sciverse.length>0 && sciverse.indexOf("gadgetId") != -1) {
        prs.rt('svAppLibs_start');
        LazyLoad.js([SDM.ep.svShindigLib, SDM.ep.svSciverseLib], function() {
          GadgetUtil.displayGadgetsWrapper();
          if(SDU.isIE7()) ArticlePage.setPaneWidths();
