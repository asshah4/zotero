...
jQuery(document).ready(function(){
	...
	jQuery('a').each(function (){
		...
		if(url.indexOf('displayFulltext') == -1 && url.indexOf('displayBackIssues') == -1 && url.indexOf('displayAbstract') == -1 && url.indexOf('displayJournal') == -1 && url.indexOf('displayIssue') == -1 && url.indexOf('displaySpecialArticle') == -1 && url.indexOf('fulltext_content')== -1){
			if(parent != 'h-menu-rl1-box' && parent != 'h-menu-rl3-box' ){
				if(url.indexOf('javascript') < 0 && url.indexOf('http') <0 && url.indexOf('#') != 0 && url.indexOf('mailto') != 0){
					...
					if (url.indexOf('?')>-1) sessIdTag='&';  
						jQuery(this).attr("href","http://journals.cambridge.org"+slash+url+sessIdTag+'sessionId='+globalSession);
