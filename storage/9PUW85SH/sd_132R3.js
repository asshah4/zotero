var AuthorAccess = {
  init: function() {
    if(SDM.authoraccess) {
      var h = '<div class="authClaim page_fragment_ind"><div class="successMsgBox">';
      h += '<div class="imgIcon">';
      ...
      h += '</a>';
      h += '</span></div></div></div>';
      $('.centerInner').prepend(h);
      $('.authClaim').css('padding-bottom', '4px');
      $('.successMsgBox').css('display','block');
    }
  },
  closeAuthClaimSuccessMsg: function() {
    $('.authClaim .successMsgBox').hide();
  }
};
...
var DynamicArtContent = {
  ...
  buildDynamicRequestData: function() {
    ...
    dynamicReqObj.isCDN = true;
    dynamicReqObj.getProdQual = true;
    dynamicReqObj.pageMetaData = requestObj;

    if ($('#checkForExcerpt')[0] || $('#excerptPlaceholder')[0]){
  ...
  populateDynamicComponents: function(fragId) {
    ...
    if ($(scope + ' #ppvPlaceHolder')[0]) {
      DynamicArtContent.buildAccessLinks(scope);
    } 
    ...
    if(typeof DynamicArtContent.dynamicComponents.prodQual != 'undefined' && DynamicArtContent.dynamicComponents.prodQual!='science') {
      DynamicArtContent.setProductQualifier($('#centerPane .head .title a'), DynamicArtContent.dynamicComponents.prodQual);
      DynamicArtContent.setProductQualifier($('#centerPane .head .articlePress a.cLink'), DynamicArtContent.dynamicComponents.prodQual);
      DynamicArtContent.setProductQualifier($('#centerPane .publicationHead p.series a'), DynamicArtContent.dynamicComponents.prodQual);
      DynamicArtContent.setProductQualifier($('#centerPane .publicationHead .volIssue a'), DynamicArtContent.dynamicComponents.prodQual);
      DynamicArtContent.setProductQualifier($('#centerPane .publicationCover a'), DynamicArtContent.dynamicComponents.prodQual);
    }

  },
  setProductQualifier:function($e, s) {
    if($e.length && typeof $e.attr('href')!='undefined') {
      var hrefStr = $e.attr('href').replace('/science/', '/' + s +'/');
      $e.attr('href', hrefStr);
    }
  },
  populateRefModFields:function() {
      if(DynamicArtContent.dynamicComponents.moduleAvail != undefined && DynamicArtContent.dynamicComponents.moduleAvail =="Y"){
         ...
         $('#mrwModuleSrcTitle').show();
         //document.getElementById('mrwModuleSrcTitle').innerHTML=$(".mrwModuleSrcTitle").html();;
         var date1 =new Date($('#hiddenDate1').text()),
             date2 =new Date($('#hiddenDate2').text());
         if (date1.getTime()<date2.getTime()) {
             $('#currentAsOf').html($('#MRDate').html());
         }
  ...
  buildPdfExcerptHtml: function(scope) {
    var excerptHTML = "<a name=\"PDFExcerpt\"></a><a class=\"excerptLink\" href=\"";
    excerptHTML += DynamicArtContent.dynamicComponents.excerptUrl;
    excerptHTML += "\">Open this preview in PDF</a>";
    $(scope).find('#excerptPlaceholder').html (excerptHTML);
    $(scope).find('#excerptPlaceholder').show();
    $(scope).find('.pdfExcerptImg').wrap('<a class="excerptLink1" href="'+DynamicArtContent.dynamicComponents.excerptUrl+'" target="_blank"/>');
    $('.excerptLink,.excerptLink1').bind('click', function(e) { var nw = window.open($('.excerptLink').attr('href'), 'newPdfWin', 'height=' + $(window).height()*.9 +',width=' + $(window).width()*.9); e.preventDefault(); e.stopPropagation();});
    if(!$('.pdfExcerptHover')[0]) {
       $('.excerptLink1').append('<div class="pdfExcerptHover"><div class="hoverContainer"><span class="hoverIcon">&nbsp;</span><span class="hoverText">Click to open first page preview</span></div></div>');
    }
  ...
  displayPdfExcerpt: function() {
    if($.browser.msie&&$.browser.version==9){
      var iframeObj="<iframe width='600' height= '820' src='" + DynamicArtContent.dynamicComponents.excerptUrl + "#toolbar=1&navpanes=0&view=Fit' frameborder='0'/>";
  ...
  buildAccessLinks: function(scope) {
    var outwardHTML='',outLinkHTML=['','','','','',''];
    var oLinks=DynamicArtContent.dynamicComponents.OutwardLinks;
    if (oLinks!=undefined && oLinks.length>0){
        for(i=0;i<oLinks.length;i++){
            if((oLinks[i].EntitlementToFullText==1) || 
               (oLinks[i].EntitlementToFullText==2 && SDM.entitled) || 
               (oLinks[i].EntitlementToFullText==3 && !SDM.entitled)){   
                    if(oLinks[i].LinkCat=='Link resolvers'){
                        outLinkHTML[0]+=DynamicArtContent.buildOutwardLinks(oLinks[i]);
                    }else if(oLinks[i].LinkCat=='Alternative platforms'){
                        outLinkHTML[1]+=DynamicArtContent.buildOutwardLinks(oLinks[i]);
                    }else if(oLinks[i].LinkCat=='Elsevier redirects'){
                        outLinkHTML[2]+=DynamicArtContent.buildOutwardLinks(oLinks[i]);
                    }else if(oLinks[i].LinkCat=='Data repositories'){
                        outLinkHTML[3]+=DynamicArtContent.buildOutwardLinks(oLinks[i]);
                    }else if(oLinks[i].LinkCat=='CitedByScopus'){
                        outLinkHTML[4]+=DynamicArtContent.buildOutwardLinks(oLinks[i]);
                    }else if(oLinks[i].LinkCat=='PermissionAndReprint'){
                        outLinkHTML[5]+=DynamicArtContent.buildOutwardLinks(oLinks[i]);
                    }
            }
        }
        outwardHTML += outLinkHTML[0] + outLinkHTML[1] + outLinkHTML[2];
    }
    if(!SDM.entitled || (DynamicArtContent.dynamicComponents.CorpReqAvail != undefined  && !SDM.ignoreForceAbst)){
        var accessHTML = '<div class="accessTitle">Choose an option to locate/access this article:</div>';
            accessHTML += '<div class="accessContent clearfix">',
            col1=false,col2=false,col3=false;
        if ( DynamicArtContent.dynamicComponents.AnonLinkAvail != undefined ) {
            accessHTML += '<div class="acol1">';  
            accessHTML += '<div style="margin-bottom:5px;">Check if you have access through your login credentials or your institution</div>';  
            accessHTML += '<a title="Click to see if you have full text access" class="corpReq cLink" queryStr="?_fmt=full" href="'
            accessHTML += SDM.urlPrefix + '/article/pii/' + SDM.pm.pii;
            accessHTML += '">Check access</a>';
            accessHTML += '</div>';
            col1=true;
        }
        if ( DynamicArtContent.dynamicComponents.CorpReqAvail != undefined ) {
            accessHTML += '<div class="acol1">';  
            accessHTML += '<div style="margin-bottom:5px;">Check if you have access through your login credentials or your institution</div>';  
            accessHTML += '<a title="Click to see if you have full text access" class="corpReq cLink" queryStr="?_fmt=full" href="'
            accessHTML += SDM.urlPrefix + '/article/pii/' + SDM.pm.pii;
        ...
        if(DynamicArtContent.dynamicComponents.PPVLinks!=undefined){
            if(col1) accessHTML +='<div class="vdivider"></div>';
            accessHTML += '<div class="acol2">';
            if ( DynamicArtContent.dynamicComponents.PPVLinks.PPVButton != undefined ) {
              accessHTML += decodeURIComponent (DynamicArtContent.dynamicComponents.PPVLinks.PPVButton);
            }
            if ( DynamicArtContent.dynamicComponents.PPVLinks.PPVPromoUrl != undefined ) {
              accessHTML += '<div class="divBlk"><div align="right" class="offerBtn" data-offerSummary="';
              accessHTML += DynamicArtContent.dynamicComponents.PPVLinks.PPVPromoUrl;
              accessHTML += '"><img src="/sd/Offer_button_03.png"/></div></div>';
            }
            if ( DynamicArtContent.dynamicComponents.PPVLinks.PPVMsg != undefined ) {
              accessHTML += '<div class="ppvmsg">' + decodeURIComponent (DynamicArtContent.dynamicComponents.PPVLinks.PPVMsg) + '</div>';
            }
            accessHTML += '</div>';
            col2=true;
        }
        if (oLinks!=undefined && oLinks.length>0){
            if ((DynamicArtContent.dynamicComponents.CorpReqAvail == undefined 
              || DynamicArtContent.dynamicComponents.CorpReqAvail == "N") 
              && (outwardHTML!='')) {
                if(col1 || col2) accessHTML +='<div class="vdivider"></div>';
                accessHTML += '<div class="acol3"><ul>' + outwardHTML + '</ul></div>';
                col3=true;
            }
        }
        accessHTML += '</div></div>';
        if(col1 || col2 || col3){
            $(scope + ' #ppvPlaceHolder').html (accessHTML).show();
            if((col1 && !col2 && !col3) || (!col1 && col2 && !col3) || (!col1 && !col2 && col3)){
                $('.acol1, .acol2, .acol3', scope+' #ppvPlaceHolder').css('width','auto');
            }
            if(col1 && col2 && !col3){
                $('.acol2', scope+' #ppvPlaceHolder').css('width','57%');
            }
            if((col1 && !col2 && col3) || (!col1 && col2 && col3)){
                $('.acol3', scope+' #ppvPlaceHolder').css('width','57%');
            }
        }
        SDM.outwardLinks ="";
    }
    if(SDM.entitled){
        var accessHTML='', custHTML='',intHTML='',altHTML='',dataHTML=''; 
        if (oLinks!=undefined && oLinks.length>0 && outwardHTML!=''){
            accessHTML += '<li class="divider">&nbsp;</li>';
            accessHTML += '<li class="accessTitle">Choose an option to locate/access this article:</li>';
            accessHTML += outwardHTML;
            SDM.outwardLinks = accessHTML;
        }
    }
    if(outLinkHTML[3]!=''){
        outLinkHTML[3]+='<li class="divider">&nbsp;</li>';
        $('#dataRepHolder').html(outLinkHTML[3]).show();
    }
    if(outLinkHTML[4]!='')$('ol.citedByLink').html(outLinkHTML[4]).show();
    if(outLinkHTML[5]!=''){
        $('.innerPaddingFixedBox .permissions').html(outLinkHTML[5]);
    }else{
        $('.innerPaddingFixedBox .permissions').hide();
    }
  }, 
  buildOutwardLinks:function(obj){
    var accessHTML = '';
        if(obj.LinkCat=='PermissionAndReprint'){
            accessHTML = '';
        }else if(obj.LinkName=='View Record in Scopus'){
            accessHTML='<li class="SC_Record">';
        }else{
            accessHTML='<li class="outLinks">';
        }
        
        if(obj.LinkURL!=undefined && obj.LinkURL!=''){
            accessHTML += '<a href="'+obj.LinkURL+'" target="_blank"';
            if(obj.LinkName!=undefined  && obj.LinkName!=''){
                accessHTML += ' title="'+obj.LinkName+'">';
            }
        }
        if(obj.Iconformat=='Y'){
            if(obj.LinkImage!=undefined  && obj.LinkImage!=''){
                accessHTML += '<span class="outImg"><img onload="DynamicArtContent.checkOLImage(this)" src="'+obj.LinkImage+'" border="0"/></span>';
            }
            if(obj.LinkName!=undefined  && obj.LinkName!=''){
                accessHTML += '<span class="outText">' + obj.LinkName +'</span>';
            }
        }else{
            if(obj.LinkImage!=undefined  && obj.LinkImage!=''){
                accessHTML += '<img onload="DynamicArtContent.checkOLImage(this)" class="outImg" src="'+obj.LinkImage+'" border="0"/>';
            }else{
                if(obj.LinkName!=undefined  && obj.LinkName!=''){
                    accessHTML += '<span class="outText">' + obj.LinkName;
                    if(obj.LinkCat=='CitedByScopus' && obj.Count!=undefined){
                        accessHTML += ' (' + obj.Count + ')';
                    }
                    accessHTML += '</span>';
                }
            }
    ...
    if(obj.LinkCat!='PermissionAndReprint')accessHTML += '</li>';
    return accessHTML;
  },
  checkOLImage:function(obj){
    if(obj.width==1 && obj.height==1){
        if($(obj).siblings('.outText').text()!=""){
            $(obj).hide();
        }else{
            $(obj).closest('.outLinks').hide();
            if($(obj).closest('ol,ul').not('ul.outwardLinks').find('li.outLinks:visible').length<=0){
                $(obj).closest('ol,ul').hide();
            }else{
                $(obj).closest('ol,ul').show();
            }
        }
    }
  },
...
//    $('#pStretcher').remove();
  ...
  fixFrag1: function() { //Called from end of frag_1 from server
    ...
    $('#centerInner').prepend($('#centerInner .headTemplate'));
    $('#frag_1').show();
    Fragment.foldCheck();
  },
  ...
  getSuccess: function(fragStr, fragID, cfg, z) {
    ...
    var prevFrag = Fragment.getPrevFrag(fragIdNum);
    prevFrag.after(fragStr);
  ...
  processFrag: function($frag) {
    ...
    if($frag.attr('data-last'))$('#footer-area').show();
  },
...
//REBASE        $(".btnHolder").bind('click',function(e){page.toggleOptions(e,this);} );
        ...
        $('#leftPane, #centerPane, #rightPane').css('top', $('#articleToolbar').outerHeight());
        $('#leftCloseBar').css('margin-top', $('#articleToolbar').outerHeight());
     ...
     submitCitation:function(e,obj) {
      ...
      setPaneWidths: function () {
        ...
        if(pgLayout.showLeftBar==false) {
          ...
          $('div#articleNav').css('margin-left',  0);
        }
        else{
          $('#leftPane,#leftCloseBar').css('left', pgLayout.lcLeft);
          $('#leftPane').css('width', pgLayout.lcWidth);
          $('div#articleNav').css('margin-left',  270);
        }
        ...
        if(SDU.isIE78())$('#rightOuter .innerPaddingFixedBox dd.doi').width($('#rightOuter .innerPaddingFixedBox dd.doi').parent().width()*.9);
      },
	...
	var imgObj=page.getCurrentImg();
      ...
      setupRelatedPDF:function() {
        $("body").on("click","#pdfLink",function(event) {
        ...
        if (SDM.tocCacheAvail) {
          if(typeof SDM.tocJson != 'undefined') {
            ...
            if($('#outline').width()>240){
              $('#outline > ul.srcOutline,#articleLeftAd').not('.taxonomy').css('width', $('#outline').width()-$.scrollBarWidth());
            }else{
              $('#outline > ul.srcOutline,#articleLeftAd').not('.taxonomy').css('width', 240);
            }
            ...
            var tocCheck = function() {
              ...
              if(typeof SDM.tocJson != 'undefined' || tocCounter>SDM.pm.toc.retry) {
                $('#centerInner').outline( $('#outline'), page.opts, SDM.tocJson );
                if($('#outline').width()>240){
                  $('#outline > ul,#articleLeftAd').not('.taxonomy').css('width', $('#outline').width()-$.scrollBarWidth());
                }else{
                  $('#outline > ul,#articleLeftAd').not('.taxonomy').css('width', 240);
                }
	...
	if(SDM.entitled==true) {
	   page.setupRelatedPDF();
        ...
        if(SDM.displayGadgets) {
          ...
          } else if (SDM.sciverseJsonObject.length > 0) {
            GadgetUtil.loadGadgets();
	  } else {
            // 
            // JSB - here is where you will want to make the AJAX
            // call to get the SDM.sciverseJsonObject.
            // Then you will want to call GadgetUtil.loadGadgets()
            //
            GadgetUtil.getJsonAndLoadGadgets();
          }
        }
    ...
    if($('#frag_1').length || !Fragment.isAvail()) {
      ...
      Fragment.get('frag_1', null, {successCb: function() {ArticlePage.addJS();}});
    }
    return page;
  }();
...
var CrossMark = {
  ...
  setCss:function(){
	...
	if(document.createStyleSheet){
     ...
     $("#crossmark-tooltip-130").css({'z-index':'1001'});
  },
  removeCss:function(){
...
var SearchWithin = {
  ...
  reqResult: function(bkOutlineSelStr, selStr, idConvert, funcNoResult) {
    $.get(SDM.urlPrefix+'/searchwithin', function(res) {
        var result;
        try {
...
var Outline = {
  ...
  buildOutlineUnavailable:function(cfg) {
    ...
    if(mrwLeftPane.currentTab!='S'){
        $('#outline .outlineMsg').show();
    }
  },
  postBuild:function(cfg) {
    ...
    var refSec = $(oObj).find ('div[id^=ol_bibl00]');
    if (refSec.length <=0) refSec = $(oObj).find ('div[id^=ol_fr]');

    if (refSec.length >= 1) {
      var refSec = refSec[0];
      var refObj = $(refSec).parent().clone();
      $(refObj).find('div.io.item').attr('id', 'toolbarRefLink');

      var txt = 'Jump to references';
      $(refObj).find('a').text(txt); 
  
      var prevObj = $('#articleNav div[id^=icon_exportarticle]');
      if ($(prevObj)[0]) {
        $(prevObj).parent('li').after(refObj);
      } else {
        $('#articleNav div.icon_pdf').parent('li').after(refObj);
      }
    
      if (typeof (Fragment) != 'undefined' && Fragment.isAvail())  {
        $("#articleNav").live("click", ".tocLink", Outline.findTargetFragment);
      } else {
        $("#articleNav").live("click", ".tocLink", Outline.moveToSection);
      }
    }
...
addItemTbl: function(hashId, lbl, hStr) {
  ...
  addItemOutline: function(hashId, lbl, hStr) {
    var html = "<li><div id='ol_" + hashId + "' class='io item " + hStr + "sec'>"
             + "<a class='cLink' queryStr='" + SDM.urlTOCLinkQueryStr + "&_hashID=" + hashId+"' href='" + SDM.urlTOCLink + "'\>"
             + lbl + "<br>"
             + "</a></div></li>";
    return html;
  }, //Outline.addOutlineItemSection
  ...
  hoverOverOn: function() {
   ...
   // ol.children('ul').css('background', '#ffffff');
    ...
    $('#outline > ul.taxonomy').css({'width':'','border':'0'});
    if(SDM.isMRWMODArticle)mrwLeftPane.setVisualCue();
  },
...
$(document).ready(function() {
    ...
    if(SDU.isIE7())$('#ieWarningMsg').ieWarning();
        //loadBreadCrumbs();
    if(SDM.secId!=''&& SDM.secId!= undefined ){
          $('#centerPane').moveTo (SDM.secId);
    }
...
var EbookTOC = {
  ...
  showSourceTab:function(e) {
    ...
    if($('#outline').scrollTop()<=0)$('#outline').scrollToTopic({selTopic:'div.activeChapter'});
  },
  ...
  buildTocNode:function(nr, lvl) {
    ...
    if(nr.PII) {
      ...
      if(nr.PII==SDM.pm.pii) {
        ...
        $node.append('<div class="' + classStr + '"><div class="' + 'srcPartStatic' + '"></div><a class="cLink" querystr="?&zone=leftPane&_origin=article&originContentFamily=nonserial" href="' + SDM.ep.urlPrefix + '/article/pii/' + nr.PII + '" title="' + nr.Title + '">' + nr.Title + '</a></div>');
      }
    }
    else if(nr.NodeList && nr.NodeList.length) {
      $node = $('<li data-pii="" class="srcli lvl' + lvl + '"></li>');
      $node.append('<div class="' + classStr + '"><div class="arrowSide leftPaneToggle">&nbsp;</div><a class = "leftPaneToggle" href="" ' + '" title="' +nr.Title + '">' + nr.Title + '</a></div>');
      var $ul = $('<ul></ul>');
  ...
  nodeClick:function(e) {
    if($(e).parent().parent().children('ul').css('display')=='none') {
      ...
      $(e).parent().children('div').addClass('arrowDown').removeClass('arrowSide');
    }
    else {
      $(e).parent().parent().children('ul').hide();
      $(e).parent().children('div').removeClass('arrowDown').addClass('arrowSide');
    }
  },
...
var ArticleToolbar = {
  ...
  init:function() {
    ...
    $('#articleNav .icon_pdf div').click(function(e){
      var currObj = e.target;
      if (SDM.entitled) {
        $(currObj).parent().find('a#pdfLink').click();
      } else {
        document.location = $(currObj).parent().find('a#pdfLink').attr("href");
      }
    });
...
function openStrippedNS(url, figElem, figRefElem, pii) {
  ...
  if(ih != null ) {
    if(cRef != null) {
      figCaption = ih.innerHTML.replace(cRef.innerHTML,'<a href="' + SDM.ep.urlPrefix + '/article/pii/' + pii + '#' + figRefElem + '">'+newRef.innerHTML+'</a>');
    }
    else {
...
var LoginBox = {
  ...
  flipLogin: function (e,button){
    ...
    this.truncateRecentInst();
  },
  truncateRecentInst: function() {
    var textContainerHeight= $('.recentInst').height();
    $('.recentInst a').each(function () {
      var $ellipsisText = $(this);
      if ($ellipsisText.outerHeight(true) > textContainerHeight) {
       ...
       $ellipsisText.parent('.recentInst').css ('height', 'auto');
      }
    });
  } 
}//LoginBox
...
var CanonicalLink = {
  init: function() {
    $("body").on("click",".cLink", function(event) {
      var t = $(this);
  ...
  if(zone == "subjectBrowse")
  {
     ...
     createCookie ("MRW_MODULE_STATE_COOKIE", cv);
      ...
      if (qs) {
        ...
        .addTag(zone, 'z')
        .addTag(qs.getParameter("_srcFr"), 'srcFr')
        ...
        .addTag(qs.getParameter("_hashID"), 'sec')
        .addTag(qs.getParameter("panel"), 'p')
        .addTag(qs.getParameter("piiSugForm"), 'sf')
        .addTag(qs.getParameter("gatewayReq"), 'gr');
...
function createCookie(cookieName, value) {
    ...
    var domain = '';
    if (hostName.indexOf ('sciencedirect.com') != -1 && hostName.match(/\.sciencedirect\.com\./) == null) {
        domain = '.sciencedirect.com';
    } else if (hostName.indexOf('.lexisnexis.com') != -1) {
        domain = '.lexisnexis.com';
    } else if (hostName.match(/\.sciencedirect\.com\./) != null) {
        domain = hostName
    }
    if (domain) {
  ...
  var helpURL = SDM.helpUrlDomain+ "/sdhelp_CSH.htm#citing_art_lrn_more.htm";
    ...
    if (obj != null) {
      ...
      $.each (obj.citation, function(i, item){
         ...
         if (enableEntitlement =='Y' && obj.ERR_TYPE!="2" && noPDF!="Y") {
            ...
            artList +='<span class="pipe">&nbsp;|&nbsp;</span><span class="pdf">'
                    + '<a ';
	    if(item.PDF_NOFOLLOW == "true")artList += 'rel="nofollow" ';
            artList +='class="cLink" aria-describedby="' +articleTitleId+ '" title="Download PDF" href="'
                    + pdfURL +'"';
            if(item.PDFURL) {
               artList += ' queryStr="' + obj.QUERYSTRING + '"';
            }
            artList +=' target="_blank">PDF&nbsp;('
                     + item.PDFSIZE + ')</a></span>';
         }
      ...
      if (jsonObj=="relatedArtList") {
           if (SDM.hpccAvail){
                artList +='more recommended articles';
            } else {
...
var rightSidePane={
  ...
  togglePane: function(actPanel) {
    if(pgLayout.showRightBar==false) {
      ...
      $('#rightOuter').show().css('margin-left',19);
      $('.toggleSideBar div').removeClass('open').addClass('close');
  ...
  getBibliographicPanel:function(obj){
    ...
    var modSrcTitle='';
    modSrcTitle=$(".publicationHead div.modSrcTitle").text();
    ...
    var htmlStr = '<dl class="citation"><dd>' + authorStr + '</dd><dd class="title">'
                      + articleTitle + '</dd><dd class="pubInfo">' ;
    if(pubTitle != undefined) {
        htmlStr += pubTitle;
    }
  ...
  getRelatedCitingPanel:function(obj){
    ...
    if(SDM.isMRWMODArticle) {
        ...
        var trd = 10;
        ///////////
        var refCheck = function() {
            clearInterval(refIntrvl);
            var conceptID,activeNode;
            if($('.taxonomy .activeNode')[0]){
                conceptID=$($('.taxonomy .activeNode')[0]).attr('href');
                activeNode=$($('.taxonomy .activeNode')[0]);
            }else{
                conceptID=$($('.taxonomy .selected')[0]).attr('href');
                activeNode=$($('.taxonomy .selected')[0]);
            }
            if(conceptID!=undefined || refCounter>trd) {
                conceptID=conceptID.substring(conceptID.indexOf('Concept-'),conceptID.indexOf('?'));
                $('#mrwTopicTitle').text('Reference articles on "'+ $(activeNode).text().replace(/^\s+|\s+$/g,'')+'"');
                var mrwRefArtURL=SDM.urlPrefix + '/mlkt/isbn/'+ SDM.pm.mod_ISBN +'/topic/'+conceptID+'/pii/'+SDM.pm.pii;
                var getArticleList=function(res){
                    if(res.TYPE!=undefined && res.TYPE!='' && res.citation.length>0){
                        rightSidePane.buildArticleList(res,"mrwRefArtList");
                    }else{
                        $('#mrwTopicTitle').parent().hide();
                    }
                };
                $.getJSON (mrwRefArtURL, getArticleList);
            }else {
                refCounter++;
                trd = trd*10;
                refIntrvl = setInterval(refCheck, trd);
            }
        }
        ///////////
        var refIntrvl = setInterval(refCheck, trd);
    }
  ...
  buildArticleList:function(res, targetId, isSpecialIssue) {
    ...
    if (res) {
      ...
      $.each (res.citation, function(i, item){
          ...
          if (res.QUERYSTRING) {
              artList += ' class="cLink" queryStr="' + res.QUERYSTRING + '"';
          }
          ...
          if (item.PDFURL) {
            artList += '<ul class="links"><li class="clearfix pdf"><div class="icon_pdf"><div class="icon"></div><a ';
	    if(item.PDF_NOFOLLOW == "true")artList += 'rel="nofollow" ';
            artList += 'title="Download PDF"  target="newPdfWin" class="cLink pdfRightPaneLink" href="';
            artList += item.PDFURL + '"';
             if(item.PDFURL) {
        ...
        switch (wsObjClass){
            ...
            case 'correspondence':
                  ...
                  if(wsObjClass!='authorName'){
                      ...
                      baseElement='.'+wsObjClass;
                      wsObj=$(evt.target).closest('li').find(baseElement);
                  }
                  if(wsObjClass=='vitae') {
                      wsObj=$('#b'+$(evt.target).attr('id')).closest('li').find(baseElement);
                  }
    ...
    populateFigures:function(selObj,selObjClass){
        ...
        if($(selObj).find('dl.figure').length>0){
            ...
            $.each ($(selObj).find("img.figure"), function(){
			    var dataGabsImgEID = $(this).attr('data-gabsEID');
                var dataFullImgEID = $(this).attr('data-fullEID');
                var dataThumbImgEID = $(this).attr('data-thumbEID');
				var dataInlImgEid = $(this).attr('data-inlimgeid');
                var dataInlImg = $(this).attr('data-inlimg');
                imgSrc= $(this).attr("src");
				if(imgSrc == '/sd/grey_pxl.gif'){
				    if (dataGabsImgEID != undefined && dataGabsImgEID != '' && SDM.entitled) {
                        imgSrc = SDM.imageUrlPrefix + $(this).attr('data-gabsEID');
                    }else if (SDM.fullSize==true && dataFullImgEID != undefined && dataFullImgEID != '') {
                       if(SDM.entitled)  {
                       imgSrc = SDM.imageUrlPrefix + $(this).attr('data-fullEID');
                       } else {
                       imgSrc = SDM.imageUrlPrefix + $(this).attr('data-thumbEID'); 
                       }
                    }else if (dataThumbImgEID != undefined && dataThumbImgEID != '') {
                        imgSrc = SDM.imageUrlPrefix + $(this).attr('data-thumbEID'); 
                    }else if (dataInlImgEid != undefined && dataInlImgEid != '') {
                        imgSrc = SDM.imageUrlPrefix + $(this).attr('data-inlimgeid'); 
	                }
				}
...
var GadgetUtil = {
  ...
  getJsonAndLoadGadgets:function() {
    SDM.sciverseDecoded=false;
    var eStr = '0';
    if(SDM.entitled) eStr='1';
    var gadgetURL = SDM.ep.urlPrefix + "/appJson/"+SDM.pm.pii+"/"+eStr;
    $.get(gadgetURL, function(data) {
      if (data) {
          SDM.sciverseJsonObject = data;      // no longer decode
          GadgetUtil.loadGadgets();
      }
    }, "text");
  },
...
/****PPVPROMO - BEGIN***/ 
$(document).ready(function(){
  $('body').on('click','.offerBtn',function(){displayOffer(this);});
});    
function displayOffer(obj){
    if($(obj).next('div.specialOffer')[0]){
        ...
        $.get(url, function(response) {
            $(obj).parents('.divBlk').append('<div class="specialOffer shadow"><span class="txtContent"></span><span class="spclose">X</span></div>');
...
var mrwLeftPane={
    ...
    load:function(){
        $('body').append('<ul id="srcOutline"></ul>');
	if(SDM.isFreeStandingNoSat){
            $('<div class="subjectMsg"><span class="iconFull infoIcon_yellow">&nbsp;</span><div class="msgTxt">Subject hierarchy temporarily unavailable.</div></div>')
	       .appendTo('#srcOutline');
	}else{
        ...
        $.get(breadCrumbTreeUrl,function(res){
            if (res && res.length > 1) {
                ...
                $('#outline').css('overflow-x', 'auto');
                $('ul.taxonomy h2').wrap('<li>');
                $('#outline').scroll(mrwLeftPane.setVisualCue);
                $('ul.taxonomy a').click(function(){
                    $('ul.taxonomy .highlight_tab').css('right','-200px');
                    setTimeout(mrwLeftPane.setVisualCue,500);
                });
                mrwLeftPane.setVisualCue();
            }else{
    ...
    setVisualCue:function(){
        var conWidth=$('#outline').width(),
            linkWidth=$('#outline a.selected:visible').outerWidth(),
            linPos=0,rightPos=0;
        if($('#outline a.selected:visible').position()){
            var linkPos=$('#outline a.selected:visible').position().left;
        }
        if($('#srcOutline').width()<$('#outline').width()){
            $('#srcOutline').css('width','100%');
        }
        var rightPos=linkWidth+linkPos-conWidth;
        if(rightPos<-10)rightPos=0;
        $('#outline a.selected div.highlight_tab').css('right',rightPos);
    },
    showSubject:function(e) {
        $(e).parent().children('.tab').removeClass('activeTab');
        $(e).addClass('activeTab');
        $('.outlineMsg').hide();
        $('#srcOutline').show().css({'display':'inline-block'});
        if(SDU.isIE7())$('#srcOutline').show().css({'display':'inline','zoom':'1'});
        $('#olGraphCbBox').hide();
        $('#outline.book').css('top', 10);
        $('#itemOutline').hide();
        mrwLeftPane.currentTab='S';
...
var RightPaneFixedBox = {
  init: function() {
    //Populate the fixed box above the accordion panel on the right pane.
    var htmlStr = '<dl class="citation">';
    //Doi 
    htmlStr += '<dd class="doi">';
    htmlStr +=    '<span>';
    htmlStr +=     '<a id="ddDoi" href="http://dx.doi.org/' + SDM.doi + '" target="doilink" onclick="var doiWin; doiWin=window.open(' + "'http://dx.doi.org/" + SDM.doi + "','doilink','scrollbars=yes,resizable=yes,directories=yes,toolbar=yes,menubar=yes,status=yes'); doiWin.focus()" + '">' + 'http://dx.doi.org/' + SDM.doi + '</a>';
    htmlStr +=     '<a class="icon_doiLink" title="Click to see how to cite or link using DOI (Opens new window)" href="' + SDM.urlPrefix + '/help/doi.htm" target="sdhelp" onmouseover="window.status=' + "'Help is Available';return true" + '" onclick="var helpWin; helpWin=window.open(' + "'" + SDM.urlPrefix + "/help/doi.htm','sdhelp','scrollbars=yes,resizable=yes,directories=no,toolbar=no,menubar=no,status=no,width=760,height=570'); helpWin.focus()" + '"></a>';
    htmlStr +=     '</span>';
    htmlStr += '</dd>';
    //CrossMark
    if (SDM.isCrossMark) {
      ...
      htmlStr += '<div>';
      htmlStr +=  '<a id="open-crossmark" href="#" style="padding: 0px 0 0px 0;"><img id="crossmark-icon" src="http://crossmark.crossref.org/images/logos/cm_sbs_018_plain.png" /></a>';  
    ...
    //OA sponsor
    if (SDM.oaSponsor != '') {
      htmlStr += '<dd class="oaSponsor">Sponsored by &nbsp;' + SDM.oaSponsor + '</dd>';
    };
    //OA user license
    if (SDM.oaUserLicense != '') {
    //htmlStr += '<dd class="oaUserLicense">' + SDM.oaUserLicense + '</dd>';
      htmlStr += '<dd class="oaUserLicense">';
      htmlStr +=    '<span>';
      htmlStr += SDM.oaUserLicense;
      htmlStr +=     '<a class="icon_oaUserLicenseLink" title="Click to see terms of use (Opens new window)" href="http://www.elsevier.com/openaccesslicenses" target="sdhelp" onmouseover="window.status=' + "'Help is Available';return true" + '" onclick="var helpWin; helpWin=window.open(' + "'http://www.elsevier.com/openaccesslicenses','sdhelp','scrollbars=yes,resizable=yes,directories=no,toolbar=no,menubar=no,status=no,width=760,height=570'); helpWin.focus()" + '"></a>';
      htmlStr +=     '</span>';
      htmlStr += '</dd>';
    };
    //Permissions and Reprints
    htmlStr += '<dd class="permissions"></dd>';
    htmlStr +='</dl>';

    $('#rightOuter .innerPaddingFixedBox').html(htmlStr);

    //Initialize crossmark stuff.
    if (SDM.isCrossMark){CrossMark.init();}
  }
}
...
var OpenAccess = {
  init: function() {
    if (SDM.isOAarticle && !SDM.isOAtitle && !($('span.articleOAstatus')[0]) ) {
      if ($('hr.artHeader')[0]) {
        //Put the OA label on the first <hr> element in the article header portion of the page. 
        $('hr.artHeader').first().wrap('<div class="OAlabel"/>');
        $('.OAlabel').prepend('<span class="articleOAstatus">&nbsp;&nbsp;Open Access</span>');
      } else {
        $('.articleOAlabelForced').html('<span class="articleOAstatus">&nbsp;&nbsp;Open Access</span><hr/>').show();
      }
    }
  }
};
