...
function HistViewTerm(term, op, num) {
    Portal.$send('AppendTerm', {'op': op, 'term': term})
}
...
jQuery(function(){
    jQuery(".img_strip p").bind("click",openCloseImageStrip);
    ...
    function openImageStrip(){
           ...
           jQuery(".img_strip_item , .img_strip_arrow").add(".img_see_all").show();
           jQuery(".img_strip").removeClass("img_strip_closed");
           jQuery(".img_strip .img_strip_header").removeClass("img_strip_closed_icon").addClass("img_strip_open_icon");
    }
    function closeImageStrip(){
            jQuery("#img_strip_Closed").val('true');
            jQuery(".img_strip_item").add(".img_strip_arrow").add(".img_see_all").hide();
            jQuery(".img_strip").addClass("img_strip_closed");
            jQuery(".img_strip .img_strip_header").addClass("img_strip_closed_icon").removeClass("img_strip_open_icon");
    }
    ...
    function moveLeftRight(e){
         ...
         if (jQuery(this).hasClass("img_strip_arrow_inactiv")){
            ...
            for (i = imageStripObj.start, j=0; j < imageStripObj.maxImagesPerView; i++, j++){
                ...
                var colegend = '<div style="display:none;" id="' + colegendId + '">' + 
                    ...
                    '<div class="legend-citation">' +
                        ...
                        ' ref="log$=image_preview_title_link&previewimagepos=' + imageStripObj.images[ind].ImagePosition +
                        '">' + decodeURI(imageStripObj.images[ind].ArticleTitle) + '</a></div>' +
                        '<div>' + imageStripObj.Citation + ' </div>' +
                    '</div> </div>';
                              
                var thumb = '<a class="figpopup" co-legend-rid="' + colegendId + '" href="' + imageStripObj.images[ind].ImageSource + '"' +
                    'ref="log$=image_preview&amp;previewimagepos=' + imageStripObj.images[ind].ImagePosition + '" ' +
                    'image-pos="' + imageStripObj.images[ind].ImagePosition  + '"' +
                    '><img src="' + imageStripObj.images[ind].ThumbUrl + '" src-large="' + imageStripObj.images[ind].ImageUrl + '"></a>' + colegend;
            ...
            window.setTimeout(function (){resizeImageStrip();},400);
            //resizeImageStrip();
    ...
    function resizeImageStrip(){
        var  minImageStripWidth = 410;
        var abstractDivWidth = jQuery(".abstract").width();
        var posssibleNumImages = parseInt((abstractDivWidth - 40) / 120);
        ...
        //console.log("dims",abstractDivWidth,posssibleNumImages,imageStripObj.imagesPerView,jQuery(".img_strip").css("width"));
        ...
        jQuery(".img_strip").css("height",(maxImgHeight + 60) + "px");
        jQuery(".img_strip_arrow").css("height",(maxImgHeight + 15) + "px");
    }
...
},
{
    ...
    PageSize: '',
    Sort: '',
    CollectionsUpperLimit: 5000,
	CollectionsUpperLimitText: '5,000',
	ClipboardLimit: 500
});
...
Portal.Portlet.Pubmed_DisplayBar = Portal.Portlet.Entrez_DisplayBar.extend({
	...
	'SendToMail': function(sendto, idx){
	    ...
	    // if ids are not selected, and an old description is present, restore that
	        ...
	        if (Portal.Portlet.Pubmed_DisplayBar.Description != '')
	            descNode.innerText = Portal.Portlet.Pubmed_DisplayBar.Description;
        }
...
function emailFormatChanged(){
    var chkSpan = jQuery(this).next();
    if (jQuery(this).val() == 'abstract' && jQuery(this).attr('options')[jQuery(this).attr('selectedIndex')].getAttribute('format') != 'text' )
        chkSpan.css('visibility','visible')
    else
        chkSpan.css('visibility','hidden')
