...
function setSearchUnchecked() {
	if (jQuery("#srch_unch").attr("checked")) {
...
function load_ads() {
	for (i=0; i<ADVERTS.length;i++) {
		...
		a.click(function() {googleTracker('/adclick'+campaign);} );
	}
}
...
jQuery(function() {
	...
	jQuery(".support-email").text("support11"+"@"+"citeulike"+".org");
	...
	jQuery(window).bind("resize", function() {
		jkmegamenu.render(jQuery);
		LOG("Re-rendering jk-menus (resize)");
	});
