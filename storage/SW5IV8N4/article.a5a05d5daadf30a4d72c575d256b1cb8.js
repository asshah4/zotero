...
function logPageView() {
	...
	var doit;
	if (GLOBAL.is_superuser) {
		doit = (ARTICLE && ARTICLE.user_article_id);
	} else {
		doit = (ARTICLE && ARTICLE.user_article_id && !ARTICLE.is_owner);
	}

	if (doit) {
		// fire and forget
		jQuery.getJSON("/logview::user_article?user_article_id="+ARTICLE.user_article_id);
	}
}
...
var citoTags = {
	...
	"includesExcerptFrom":	"providesExerptFor",
	...
	"providesExerptFor":	"includesExcerptFrom",
...
function addCitoTag(doReverse) {
	...
	var reverseCito = citoTags[val];

	if (!doReverse) {
		jQuery("#cite-reverse-type").text(reverseCito);
...
function setupCito() {
	...
	// tags are lowercase only, cito is camelCase.  So make a map from
	// lowercase -> camelCase
	var nameMap = {};
	for (var c in citoTags) {
		nameMap[c.toLowerCase()] = c;
	}
...
function openurlResolverYQL(openurl_ip, ip,doi,pmid) {
	...
	//var url = "http://example.com/?callback=?";
	try {
		jQuery.getJSON(url)
			.success(function(data) {
				if (data && data.query && data.query.count && data.query.count == 1) {
					...
					/*
						jQuery("#openurlResolverName").text("GetCopy OpenURL Resolver");
						jQuery("#openurlResolverImage").attr("src",GLOBAL.imgRoot+"/getcopy.gif");
						jQuery("#openurlResolverLink").attr("href","http://getcopy.edina.ac.uk/getcopy/getcopy.asp?"+jQuery.param({"rft_id":rft_id, "rft.genre":"article"}));
						jQuery("#openurlResolver").show();
					*/
					}
...
/*
 Possible test for DnD file upload compatibility
*/
function MultipleFileCompat(){
...
// Or this?
function initHtml5FileUpload() {
	// Don't think this is reliable.
