...
if (AuthorArticleList == null) var AuthorArticleList = {};
AuthorArticleList._path = '/dwr';

AuthorArticleList.getListOfAuthorArticles = function(p0, p1, callback) {
  dwr.engine._execute(AuthorArticleList._path, 'AuthorArticleList', 'getListOfAuthorArticles', p0, p1, callback);
}

function getListOfAuthorArticles(journal, author) {
     AuthorArticleList.getListOfAuthorArticles(journal, author, authorList);
} 
   
function authorList(list) { 
	datalog = "<a href=#  onclick=javascript:hideDiv('authorList');return false;><div id='author_close_div'><img id='author_close' src='/images/clear.gif' /></div></a><p><div id='author_results'><ul>" + list+ "</ul></div>";
	document.getElementById('authorList').style.display = 'block';
	document.getElementById('authorList').style.height = 'auto';	
	DWRUtil.setValue("authorList", datalog, { escapeHtml:false });
}
...
dwr.engine._origScriptSessionId = "31D50418F72B5CA6D2388004A4193F37";
...
dwr.engine._defaultPath = "/eclips/dwr";
...
function showHideCollexMessage(pii) {
	jQuery("#upsellcollectiontopics").slideToggle('normal');
		document.getElementById('upsellCollectionList').innerHTML ='<span>Loading Collections List...</span><img src="/images/loader.gif">';
		document.getElementById('upsellCollectionButton').style.display = 'none';
	    getUpsellCollection(pii);
	}

	function setpriceCollex(content) {
		document.getElementById('upsellCollectionList').innerHTML = content;
		if (content.indexOf("Please try later") > -1) {
			document.getElementById('upsellCollectionButton').style.display = 'none';
		} else {
			document.getElementById('upsellCollectionButton').style.display = 'block';
		}
	}

	function setupCollexCheckbox(obj) {
		var objSub = document.getElementsByName("subcollectionCode");
		for (var x = 0; x < objSub.length; x++) {
			if (objSub[x].id == obj.value) {
				if (obj.checked) {
					objSub[x].checked = false;
					objSub[x].disabled = true;
					checkChildAndDisable(objSub[x].id, objSub[x].value);
				} else {
					var child = checkSimilarChildExists(objSub[x].id, objSub[x].value);
					if (child != null) {
						if (!checkParentChecked(child.id)) {
							child.disabled = false;
							objSub[x].disabled = false;
						}
					} else {
						objSub[x].disabled = false;
					}
	...
	function disableDuplicateCheck(obj) {
		var objSub = document.getElementsByName("subcollectionCode");
		for (var x = 0; x < objSub.length; x++) {
			if (obj.checked) {
				if (objSub[x].value == obj.value && objSub[x].id != obj.id) {
					objSub[x].checked = false;
					objSub[x].disabled = true;
				}
			} else {
	...
	function checkParentChecked(childId) {
		var objPar = document.getElementsByName("collectionCode");
		for (var p = 0; p < objPar.length; p++) {
			if (objPar[p].value == childId) {
				return objPar[p].checked;
			}
		}
	...
	function checkChildAndDisable(childId, childValue) {
		var child = checkSimilarChildExists(childId, childValue);
		if (child != null) {
			child.checked = false;
			child.disabled = true;
		}
	...
	function checkChildAndEnable(childId, childValue) {
		var child = checkSimilarChildExists(childId, childValue);
		if (child != null) {
			child.disabled = false;
		}
	...
	function checkSimilarChildExists(childId, childValue) {
		var objChild = document.getElementsByName("subcollectionCode");
		for (var c = 0; c < objChild.length; c++) {
			if (objChild[c].value == childValue && objChild[c].id != childId) {
				return objChild[c];
			}
		}
		return null;
	...
	function enableDisableButton() {
		...
		var parentCheck = document.getElementsByName("collectionCode");
		var subCheck = document.getElementsByName("subcollectionCode");
		for ( var x = 0; x < parentCheck.length; x++) {
			if (parentCheck[x].checked) {
				buttonFlag = true;
				break;
			}
		}
		
		for (var x = 0; x < subCheck.length; x++) {
			if (subCheck[x].checked) {
				buttonFlag = true;
				break;
			}
		}
	...
	function addToCart() {
			var parentCollection = document.getElementsByName("collectionCode");
			var subCollection = document.getElementsByName("subcollectionCode");
			var parentCategoryValue="";
			var subCategoryValue="";
			var parentValue="";
			var subValue="";
			if(parentCollection)
				{
					for ( var x = 0; x < parentCollection.length; x++) 
						{
	                    	if (parentCollection[x].checked == true)
	                    	{
	                                parentCategoryValue = parentCollection[x].value;
	                                parentValue += parentCategoryValue + ',';
	                    	}
	                    }
	         ...
	         if(subCollection)
	          {
	          	for ( var x = 0; x < subCollection.length; x++) 
					{
	                    	if (subCollection[x].checked == true)
	                    	 {
	                                subCategoryValue = subCollection[x].value;
	                                subValue +=  subCategoryValue + ',';
	                    	}
	                }
	          }  
			
	        if(parentValue !="" || subValue != "" )
	         { 
	        	document.upsellCollectionForm.action= "/collectionsubscribe?collexcode="+parentValue+"&subcollexcode="+subValue;
	        	document.upsellCollectionForm.submit();   
	         }  
	    }
...
function toggleImage(figureId, smallImage, largeImage, largeImageWidth, largeImageHeight) {

	image = document.getElementById(figureId + '_version');
	sizer = document.getElementById(figureId + '_sizer');
	sizelink = document.getElementById(figureId + '_sizelink');

	oldsource = image.src;
	sizelink.href = oldsource;
	var oldsizesrc = sizer.src;
	
	if (oldsizesrc.indexOf("plus")!=-1){
		image.src = oldsource.replace(smallImage,largeImage);
		sizer.src = '/images/large_image_minus.gif';
		if( largeImageWidth > 745) {
			document.getElementById(figureId + '_container').style.overflowX = 'scroll';
		}
	}
	else {
		image.src = oldsource.replace(largeImage,smallImage);
		sizer.src = '/images/large_image_plus.gif';
		document.getElementById(figureId + '_container').style.overflowX = 'hidden';
	}
}
